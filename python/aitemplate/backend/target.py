...
class Target(object):
    ...
    def in_ci_env(self) -> Union[None, str]:
        ...
        """
        return os.environ.get("CI_FLAG", None) == "CIRCLECI"
    ...
    def _load_profile_cache(self):
        ...
        try:
            ...
            logger.info(__name__, f"Cannot mkdir at {prefix} due to permission issue")
            prefix = os.path.join("/tmp", ".aitemplate")
            os.makedirs(prefix, exist_ok=True)
        ...
        logger.info(__name__, "Loading profile cache from: %s" % cache_path)
