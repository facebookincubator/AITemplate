...
__global__ void nhwc_to_nchw_kernel(T *output,
  ...
  const int32_t wid  = tid / 32;//th.y:0-7
  const int32_t lid  = tid % 32;//th.x:0-31
  const int32_t ni0   = blockIdx.z;
  const int32_t hwi0  = blockIdx.y * 32;//parallel 8*seq 4
  const int32_t ci0 = blockIdx.x * 32;//parallel 32
  ...
  if (ci0 + lid < c) {
    ...
    if ((hwi0 + 32) <= hw) {
      ...
      for (int cLoopIdx = 0; cLoopIdx < 4; cLoopIdx++) {
        ...
        A                     = &A[8 * c];//because c is distributed on threads y
        hwi += 8;
      }
    } else {
  ...
  if(hwiOut < hw){
    if(ci0 + 32 < c){
      ...
      for(int hwLoopIdx = 0; hwLoopIdx < 4; ++hwLoopIdx){
          output[ni0*c + ci0 + cI] = shbuf[(cI)*33 + lid];
          cI+=8;
      }
    } else {
