...
//
// Assuming input size: [M, K], we would like to performan the reduciton among
// columns, i.e. K. we use vector read with pack_size as length. When K is
// small, we use wrapReduce and store results in logcal registers; When K is
// large, we use blockReduce and store to shared memory. For the detailed
// threadhold K, please refer to this post: https://fb.quip.com/HCfIAbpWB0qi and
// this experiment log:
// https://docs.google.com/spreadsheets/d/1bl3GCLQ67p27kXOSVJikEob38fojqaZIS--mPdQxeo0/edit#gid=931264442
...
template <typename T>
__global__ void
softmaxBlockNocache(T* input, T* output, size_t m, const size_t n) {
...
__global__ void
softmax_block_smem(const T* input, T* output, size_t m, const size_t n) {
  const int read_t_sz = sizeof(T);
  const int act_t_sz = sizeof(ACT_T);
  const int pack_size = read_t_sz / act_t_sz;
