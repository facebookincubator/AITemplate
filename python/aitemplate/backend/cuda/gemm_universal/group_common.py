...
def gen_function(
    ...
    for i in range(func_attrs["groups"]):
        operand = []
        operand.append(f"ptr_{i}_a")
        operand.append(f"ptr_{i}_b")
        operand.append(f"ptr_{i}_c")
        if has_bias:
            operand.append(f"ptr_{i}_bias")
        dims = []
        for j in range(6):
            dims.append(f"dim_{i}_{j}")
