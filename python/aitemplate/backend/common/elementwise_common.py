...
KERNEL_READ_INPUTS_TEMPLATE = jinja2.Template(
    """
  {% for broadcast_size in broadcast_sizes %}
  {% if broadcast_size == "" %}
  {{read_t}} tmp_i{{loop.index0}} = input{{loop.index0}}[idx];
  {% else %}
  {{read_t}} tmp_i{{loop.index0}} = input{{loop.index0}}[({{broadcast_size}}) / N_ELEMENTS_PER_THREAD];
  {% endif %}
  {% endfor %}

  {% for broadcast_size in broadcast_sizes %}
  const {{op_t}}* p_tmp_i{{loop.index0}} = reinterpret_cast<const {{op_t}}*>(&tmp_i{{loop.index0}});
  {% endfor %}
...
KERNEL_WRITE_OUTPUTS_TEMPLATE = jinja2.Template(
    """
  {% for output_accessor in output_accessors %}
  {% if output_accessor.is_contiguous %}
  output{{loop.index0}} = get_strided_address</*data_t*/ {{data_t}},
                                              /*read_t*/ {{read_t}},
                                              /*is_contiguous*/ true>(
      output{{loop.index0}}, idx, {{output_accessor.offset}}, 0, 0);
  {% else %}
  output{{loop.index0}} = get_strided_address</*data_t*/ {{data_t}},
                                              /*read_t*/ {{read_t}},
                                              /*is_contiguous*/ false>(
      output{{loop.index0}}, idx,
  ...
  {% endif %}
  *output{{loop.index0}} = tmp_o{{loop.index0}};
  {% endfor %}
    """
)
...
class FusedElementwiseMetaData:
    ...
    inputs: List[Tensor]
    outputs: List[Tensor]
    # TODO: Add input_accessors support.
...
def _get_types_and_sizes(
    ...
    for input_tensor in inputs:
        ...
        input_shape = input_tensor._attrs["shape"]
...
def _parse_func_metadata(
    ...
    alignment, input_broadcast_sizes, dtype = _get_types_and_sizes(
        inputs, output_accessors, backend_spec
    )
...
def _gen_kernel_function(
    ...
    read_inputs = KERNEL_READ_INPUTS_TEMPLATE.render(
    ...
    write_outputs = KERNEL_WRITE_OUTPUTS_TEMPLATE.render(
        output_accessors=fused_elementwise_metadata.output_accessors,
        read_t=fused_elementwise_metadata.read_t,
