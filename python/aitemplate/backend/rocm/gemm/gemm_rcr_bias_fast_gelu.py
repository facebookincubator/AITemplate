...
struct AddFastGelu
{
    ...
    {
        const float a = x0 + x1;
        const float u   = float(2) * a * (float(0.035677) * a * a + float(0.797885));
    ...
    operator()<double>(double& y, const double& x0, const double& x1) const
    {
        ...
        const double u   = double(2) * a * (double(0.035677) * a * a + double(0.797885));
        const double emu = exp(-u);
        const double cdf = double(0.5) + double(0.5) * (double(2) / (double(1) + emu) - double(1));
        y = a * cdf;
    };
