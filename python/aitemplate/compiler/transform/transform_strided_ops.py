...
def _gemm_stride_checker(original_tc: TensorAccessor, dim: int) -> bool:
    if not _dynamic_shape_checker(original_tc.original_shapes, dim):
        return False
    ...
    tmp_tc = copy.deepcopy(original_tc)
    tmp_shape = copy.deepcopy(
        original_tc.actual_shapes
        if original_tc.actual_shapes is not None
        else original_tc.original_shapes
    )
    ...
    tmp_tc.update_base_tensor(tmp_tensor, dim, stride_dim_offset=0)
    stride_strs = tmp_tc.try_get_stride_strs(len(original_tc.original_shapes) - 2)
    if stride_strs is None:
        logger.debug(
            f"Failed in gemm_stride_checker: "
            f"dim: {dim}, "
            f"original_shapes length: {len(original_tc.original_shapes)}"
        )
        return False
    else:
        return True
...
def _is_valid_for_fusion(strided_op: Operator, cat_op: Operator, out_idx: int):
    ...
    if _is_reduce_op(op_type):
        return _reduce_cat_checker(cat_op)
    return True
...
def _fuse_strided_op_and_cat(sorted_graph: List[Tensor]) -> List[Tensor]:  # noqa: C901
    ...
    for op in sorted_ops:
        ...
        if not transform_utils.cat_split_dim_is_static(cat_op, cat_dim):
            continue
...
def _fuse_group_gemm_reshape_cat(sorted_graph: List[Tensor]) -> List[Tensor]:
    ...
    for op in sorted_ops:
        ...
        if not _is_strided_group_gemm(op):
            continue
        strided_op = op
        output_idx_to_be_updated = []
        view_op_to_be_removed = []
        cat_op = None
        for idx, output_tensor in enumerate(strided_op._attrs["outputs"]):
            ...
            view_op_to_be_removed.append(view_op)
            offset = 0
            ...
            output_idx_to_be_updated.append(idx)
            cat_output._attrs["src_ops"].add(strided_op)

        if cat_op:
            cat_output = cat_op._attrs["outputs"][0]
            for view_op in view_op_to_be_removed:
                transform_utils.remove_view_op_from_sorted_graph(view_op)

            for idx in output_idx_to_be_updated:
...
def transform_strided_ops(sorted_graph: List[Tensor]) -> List[Tensor]:
    """
