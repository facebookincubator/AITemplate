...
class TensorUsageRecord:
    """
    [summary] A named tuple to keep a tensor usage record, where
...
def _find_original_tensor(tensor: Tensor):
    """[summary] Find the original tensor of a tensor view recursively.
...
def _make_tensor_usage_records(sorted_ops: List[Operator]):
    """[summary] Create a list of tensor usage records for the given operators.

    Parameters
    ----------
    sorted_ops : List[Operator]
        [description]
