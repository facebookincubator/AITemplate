...
"""
[summary] Perform operator fusions.
"""
...
def _fuse_layernorm_sigmoid_mul(sorted_graph: List[Tensor]) -> List[Tensor]:
    ...
    for tensor in sorted_graph:
        ...
        if src_op._attrs["op"] != "layernorm":
            continue
        layer_norm = src_op
        layer_norm_shapes = tensor._attrs["shape"]
        if len(layer_norm_shapes) != 2:
            continue
...
def fuse_ops(sorted_graph: List[Tensor]) -> List[Tensor]:
    funcs = [
