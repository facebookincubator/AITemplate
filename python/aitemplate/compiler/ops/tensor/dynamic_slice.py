...
class dynamic_slice(Operator):
    ...
    def _infer_shapes(
        self, x: Tensor, start_indices: List[int], end_indices: List[int]
    ) -> List[IntVar]:
    ...
    def __call__(
        ...
        start_indices = [idx if idx is not None else 0 for idx in start_indices]
        end_indices = [idx if idx is not None else MAX_INT32 for idx in end_indices]
