...
class gemm(Operator):
    ...
    def _extract_epilogue_alignment(self, output_shape: List[IntVar]) -> None:
        ...
        if not isinstance(epilogue_dim, IntImm):
            raise RuntimeError(
                "Gemm output last dimension must be static! gemm: {}".format(gemm)
            )
        ...
        if not target.use_dummy_profiling_results():
            ...
            for wkl in workloads:
                ...
                if cache_value is not None and not target.force_profile():
                    logger.info(
                        __name__, f"Load profiling result from cache: {cache_value}"
                    )
    ...
    def gen_profiler(
        ...
        ab_alignments = sorted(set(self._get_ab_alignment(wkl) for wkl in workloads))
        assert 1 == len(
            ab_alignments
