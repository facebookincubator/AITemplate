...
class reshape(_reshape_base):
    ...
    def gen_function_decl(self) -> str:
        target = backend.target.Target.current()
        func_key = "{target}.{op}.func_decl".format(
            target=target.name(), op=self._attrs["op"]
        )
        func = registry.get(func_key)
    ...
    def gen_function_call(self) -> str:
        target = backend.target.Target.current()
        func_key = "{target}.{op}.func_call".format(
            target=target.name(), op=self._attrs["op"]
        )
        func = registry.get(func_key)
        return func(self._attrs)
...
class squeeze(_view):
    ...
    ----------
    dim : Optional[int]
        The dimension to get rid of. If None, get rid of all dimensions of size 1.
    """
...
class unsqueeze(squeeze):
    ...
    ----------
    dim : int
        Where to add the dimension, must be in range [-input_ndim - 1, input_dim + 1)
    """
