...
class Proposal(Module):
    ...
    def box_transform(self, bbox_deltas, anchors):
        ...
        if self.clip_box:
            ...
            proposals_c = ops.concatenate()([f_x1, f_y1, f_x2, f_y2], dim=1)
        else:
            proposals_c = ops.concatenate()([p_x1, p_y1, p_x2, p_y2], dim=1)

        proposals = ops.elementwise(FuncEnum.CLAMP_NAN_TO_NUM)(
            proposals_c,
            Tensor(value=-1000.0999755859375, shape=[]),
            Tensor(value=1000.099975585937, shape=[]),
            Tensor(value=0.0, shape=[]),
        )
