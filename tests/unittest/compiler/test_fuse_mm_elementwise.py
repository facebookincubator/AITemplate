...
class FuseGemmRcrBiasCase(unittest.TestCase):
    ...
    def test_gemm_rcr_bias_add_fail(self):
        ...
        module = gen_execution_module(
            output, target, "./tmp", "gemm_bias_fusion_add_fail"
        )
        ...
        y = torch.empty([M, N]).cuda().half()
        module.RunWithTensors([X_pt, W_pt, B_pt, B1_pt], [y])
        self.assertTrue(torch.allclose(Y_pt, y, atol=1e-1, rtol=1e-1))
    ...
    def test_gemm_rcr_bias_chained(self):
        ...
        target = detect_target()
        module = gen_execution_module(
            output, target, "./tmp", "gemm_bias_fusion_chained"
        )
        ...
        y = torch.empty([M, N]).cuda().half()
        module.RunWithTensors([X_pt, W_pt, B_pt, D_pt], [y])
        self.assertTrue(torch.allclose(Y_pt, y, atol=1e-1, rtol=1e-1))
    ...
    def test_gemm_rcr_bias_fail(self):
        ...
        # Check value correctness
        target = detect_target()
        module = gen_execution_module(output, target, "./tmp", "gemm_bias_fusion_fail")
        ...
        y = torch.empty([M, N]).cuda().half()
        module.RunWithTensors([X_pt, W_pt, B_pt], [y])
        self.assertTrue(torch.allclose(Y_pt, y, atol=1e-1, rtol=1e-1))
...
class FuseBmmCcrAddCase(unittest.TestCase):
    ...
    def _test_bmm_ccr_add_negative(self, testname, negative_type):
        ...
        # Check value correctness
        target = detect_target()
        module = gen_execution_module([output, output_1], target, "./tmp", testname)
