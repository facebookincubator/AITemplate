...
class ConstantFoldingTestCase(unittest.TestCase):
    def _verify_graph(
        ...
        num_constants = sum(
            1 for tensor in mod.debug_sorted_graph if isinstance(tensor, ConstantTensor)
        )
    ...
    def test_simple_constant_fold(self):
        ...
        inp0_ait = ConstantTensor(
            TorchConstantTensorData(inp0_pt), shape=(3, 3), name="inp0"
        )
        inp1_ait = ConstantTensor(
            TorchConstantTensorData(inp1_pt), shape=(3, 3), name="inp1"
        )
        ...
        target = detect_target()
        mod = gen_execution_module(
            y_ait, target, "./tmp", "test_constant_folding_simple"
        )

        y = torch.empty((9,)).cuda().half()
        mod.RunWithTensors({"inp2": inp2_pt}, {"y": y})
        self.assertTrue(torch.equal(y, y_pt))

        # Make sure we eliminated the first elementwise op. We start with 7
        # tensors, eliminate the elementwise op + its 2 inputs + one flatten,
        # and add one constant, so the total size should be 4.
        self._verify_graph(mod, expected_num_constants=1, expected_num_nodes=4)

    def test_pad_constant_weight(self):
        M, N, K = 16, 32, 3
        w_pt = torch.randn((K, N)).half().cuda()
        weight_data = TorchConstantTensorData(w_pt)
        input_0 = Tensor(shape=[M, K], name="input_0", is_input=True)
        W = ConstantTensor(weight_data, shape=[K, N], name="weight")
        Y = ops.gemm_rrr()(input_0, W)
        Y._attrs["name"] = "y"
        Y._attrs["is_output"] = True

        target = detect_target()
        mod = gen_execution_module(Y, target, "./tmp", "test_pad_constant_weight")

        input_0_pt = torch.randn((M, K)).half().cuda()
        y_pt = torch.matmul(input_0_pt, w_pt)

        y = torch.empty((M, N)).cuda().half()
        mod.RunWithTensors({"input_0": input_0_pt}, {"y": y})
    ...
    def test_fold_long_chain(self):
        ...
        w1_data = TorchConstantTensorData(w1_pt)

        w2_pt = torch.randn((K, N)).half().cuda()
        w2_data = TorchConstantTensorData(w2_pt)

        w3_pt = w1_pt * w2_pt
        x_pt = torch.randn((M, K)).half().cuda()
        x_pt_data = TorchConstantTensorData(x_pt)

        y_pt = torch.matmul(x_pt, w3_pt)
        w4_pt = torch.randn((M, N)).half().cuda()
        w4_data = TorchConstantTensorData(w4_pt)
        z_pt = y_pt * w4_pt

        w1_ait = ConstantTensor(w1_data, shape=[K, N], name="w1")
        w2_ait = ConstantTensor(w2_data, shape=[K, N], name="w2")
        w3_ait = ops.elementwise(FuncEnum.MUL)(w1_ait, w2_ait)
        x_ait = ConstantTensor(x_pt_data, shape=[M, K], name="x")
        y_ait = ops.gemm_rrr()(x_ait, w3_ait)
        w4_ait = ConstantTensor(w4_data, shape=[M, N], name="w4")
        z_ait = ops.elementwise(FuncEnum.MUL)(y_ait, w4_ait)
        z_ait._attrs["name"] = "z"
        z_ait._attrs["is_output"] = True

        target = detect_target()
        mod = gen_execution_module(z_ait, target, "./tmp", "test_pad_constant_weight")

        z = torch.empty((M, N)).cuda().half()
        mod.RunWithTensors({}, {"z": z})
    ...
    def test_late_binding(self):
        # Test binding constants through gen_execution_module
        M, N, K = 16, 32, 3
        w1_pt = torch.randn((K, N)).half().cuda()
    ...
    def test_late_binding_error_constant_already_bound(self):
        ...
        w1_ait = ConstantTensor.create_host_zero_tensor(shape=[K, N], name="w1")
        w2_ait = Tensor(shape=[K, N], name="w2")
    ...
    def test_late_binding_fails_wrong_dtype(self):
        ...
        for w1_pt in wrong_inputs:
            with self.assertRaisesRegex(
                ValueError,
                r"ConstantTensor's dtype did not match: expected float16, got .*",
            ):
                target = detect_target()
                gen_execution_module(
                    y,
