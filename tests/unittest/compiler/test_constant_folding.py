...
class ConstantFoldingTestCase(unittest.TestCase):
    ...
    def test_constant_folding_with_update(
        ...
        const_folder_constants["constant_3"] = get_random_torch_tensor(
            (1, 2), "float16"
        )
        const_folder_unbound_constants["constant_2"] = get_random_torch_tensor(
            (1, 2), "float16"
        )
        ...
        mod = compile_model(
            ...
            detect_target(),
            "./tmp",
            f"test_unbound_constant_{update_model_bound}_{update_model_unbound}_{update_const_folder_bound}_{update_const_folder_unbound}_{double_buffer}",
        ...
        def _get_output(new_bound_constants, new_unbound_constants):
            ...
            x1_pt = torch.cat((x_pt, x_pt, new_bound_constants["constant_1"]))
            y_pt = torch.cat(
                (
                    new_unbound_constants["constant_2"],
                    new_bound_constants["constant_3"],
                    new_unbound_constants["constant_4"],
                )
            )
            output_pt = x1_pt * y_pt
            return output_pt
