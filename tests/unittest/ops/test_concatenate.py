...
class ConcatenateTestCase(unittest.TestCase):
    ...
    def _run_concatenate(
        ...
        module = gen_execution_module(Y, target, "./tmp", "concatenate")

        input_tensors_ait = {
            f"input_{idx}": input_tensors_pt[idx] for idx in range(len(inputs))
        }
        y = torch.empty(y_shape).cuda().half()
        module.RunWithTensors(input_tensors_ait, [y])
    ...
    def _run_batch_concatenate(
        ...
        module = gen_execution_module(
            Y, target, "./tmp", f"concatenate_batched_{batch_tag}"
        )
    ...
    def _run_masked_concatenate(
        ...
        *,
        ...
        # setup new input_masks and inputs
        ...
        module = gen_execution_module(Y, target, "./tmp", "concatenate_masked")
