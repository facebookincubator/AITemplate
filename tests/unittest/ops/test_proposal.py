...
class ProposalTestCase(unittest.TestCase):
    def test_fp16_single_op(self, test_name="proposal"):
        ...
        anchors = OP._anchors.copy()
        batch_inds = OP._batch_inds.copy()
        module.SetConstant("anchors", anchors)
        module.SetConstant("batch_inds", batch_inds)
        inputs_pt = [
            torch.from_numpy(bbox_deltas_ait).cuda().half(),
            torch.from_numpy(scores_ait).cuda().half(),
        ]
        out0_shape = module.GetOutputMaximumShape(0)
        out0 = torch.empty(out0_shape).cuda().half()
        y_ait_shape = module.GetOutputMaximumShape(1)
        y_ait = torch.empty(y_ait_shape).cuda().half()
        module.RunWithTensors(inputs_pt, [out0, y_ait])
