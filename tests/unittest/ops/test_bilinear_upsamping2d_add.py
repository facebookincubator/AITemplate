...
class BilinearUpsamplingAddTestCase(unittest.TestCase):
    def _test_fp16_single_op(
        self,
        scale_factor=2.0,
        batch_size=_DEFAULT_BATCH_SIZE,
        test_name="bilinear_upsampling2d_add",
    ):
        channels = 1024
        HH, WW = 8, 8
        ...
        OP = nn.BilinearUpsampling2dAdd(scale_factor=scale_factor, mode="bilinear")
        Y = OP(X, R)
        ...
        for b in batch_size:
            ...
            module.RunWithTensors({"input_0": x, "input_1": r}, [y])
