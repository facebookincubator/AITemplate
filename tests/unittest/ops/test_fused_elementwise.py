...
class FusedElementwiseTestCase(unittest.TestCase):
        ...
        for batch_size in batch_sizes:
            for m in ms:
                for k in ks:
                    ...
                    module.RunWithTensors([x1_pt], [x4])
    ...
    def test_fused_elementwise_kernel1(self):
        ...
        target = detect_target()
        module = gen_execution_module(X9, target, "./tmp", "fused_elementwise_kernel1")
    ...
    def _test_min_max(
        ...
        x2 = torch.empty(input_size).cuda().half()
        module.RunWithTensors(inputs, [x2])
        x2 = x2.cpu().numpy()
    ...
    def _test_clamp(
        ...
        module.RunWithTensors([x0_pt], [x1])
