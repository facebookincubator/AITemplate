...
class StableDiffusionAITPipeline:
    def __init__(
        ...
        if ckpt is None:
            self.clip_pt = CLIPTextModel.from_pretrained(hf_hub_or_path, subfolder="text_encoder", revision="fp16", torch_dtype=torch.float16).cuda()
        else:
            config = CLIPTextConfig.from_pretrained(hf_hub_or_path, subfolder="text_encoder")
        ...
        if ckpt is None:
            self.unet_pt = UNet2DConditionModel.from_pretrained(hf_hub_or_path, subfolder="unet", revision="fp16", torch_dtype=torch.float16).cuda()
        ...
        if ckpt is None:
            self.vae_pt = AutoencoderKL.from_pretrained(hf_hub_or_path, subfolder="vae", revision="fp16", torch_dtype=torch.float16).cuda()
        else:
