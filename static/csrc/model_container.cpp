...
ModelContainer::ModelContainer(
    ...
    size_t num_outputs,
    size_t params_size)
    : ModelContainerBase(num_inputs, num_outputs, params_size),
...
void ModelContainer::RunWithOutputsOnHost(
  ...
  std::vector<AITemplateTensor> owned_outputs;
...
float ModelContainer::Benchmark(
  ...
  std::vector<std::vector<AITemplateTensor>> per_thread_outputs;
  ...
  for (size_t i = 1; i < num_threads; ++i) {
    ...
    std::vector<AITemplateTensor> cloned_outputs;
  ...
  auto thread_func = [&](size_t thread_idx) {
    AITemplateTensor* thread_outputs =
...
void ModelContainer::SetConstant(
    const char* name,
    const void* src,
    size_t size) {
  auto it = constant_name_to_offset_.find(name);
  if (it == constant_name_to_offset_.end()) {
    // TODO make this an exception after we fix the CMF benchmarks
    LOG(ERROR) << "Constant " << name << " not found";
    return;
  }
  auto offset = it->second;
  auto* dst = static_cast<uint8_t*>(constants_.get()) + offset;
  DEVICE_CHECK(CopyToDevice(dst, src, size));
}
